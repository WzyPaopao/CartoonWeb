...
<%
	...
	int chapterNum = Integer.parseInt( session.getAttribute("chapterNum").toString() );
%>
...
<body>
	...
	<% if( chapterNum  < thebook.getDic().size() - 1 && chapterNum > 0 ){ %>
		<a href="./viewBookServlet?chapterNum=<%= chapterNum - 1 %>" class = "former">上一章</a>|
		<a href="./viewBookServlet?chapterNum=<%= chapterNum + 1 %>" class = "later">下一章</a>|
	<%}else if( chapterNum == thebook.getDic().size() - 1 ){ %>
		<a href="./viewBookServlet?chapterNum=<%= chapterNum - 1 %>" class = "former">上一章</a>|
